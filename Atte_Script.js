...
function calculateTimeDifference() {
    ...
    var x = `${diff_OT_Hours}h, ${diff_OT_Minutes}m, ${diff_OT_Seconds}s`;
...
let keyLen = localStorage.length;
console.log(keyLen);

function getLocalData() {

    function crDiv(stCd, stNm, stDt, stOt, stTi, stTo, stWt) {
        let Ot;
        if (stOt) {
            Ot = stOt;
        } else {
    ...
    for (let i = keyLength; i >= 1; i--) {
        ...
        const stNm = str => str.charAt(0).toUpperCase()+str.slice(1);
        setName = stNm(setName);
        
        let setDate = dT.Att_Date;
        console.log(setDate);
        let setDtlen = setDate.length;
        console.log(setDtlen);
        sendDatefunc(setDate);
        ...
        for (let  ind = 0; ind <= setDtlen-1; ind++){
            console.log(ind);
            let getDate = setDate[ind];
            console.log(getDate);
            let getDateByJson = JSON.parse(getDate);
            console.log(getDateByJson);
            let callDiv = crDiv(setCode, setName, getDateByJson["Date"], getDateByJson["overTime"], getDateByJson["timeIn"], getDateByJson["timeOut"], getDateByJson["workTime"]);
            console.log(callDiv);
            callDivG = callDiv;
            // ind--;
            console.log(ind);
        ...
        console.log(callDivG);
    	person.append(callDivG);
...
function object() {
    try {
        ...
        let dateObjWithJson = [];
        if (existingIds.has(codein.value)) {
                        
            dateObj = JSON.stringify(dateObj);
            dateObjWithJson.push(dateObj);

            objreturnDate = JSON.stringify(objreturnDate);
            dateObjWithJson.push(objreturnDate);
            
            console.log(dateObjWithJson);
            Number(vari--);
            console.log(vari);
            console.log("it's working");
        }else{
            dateObjWithJson = dateObj;
        };
        
        const objA = new MyDataClass(codein.value, namein.value, dateObjWithJson);
        if(existingIds.has(objA["EmpCode"])){
            vari-1;
        };
...
xlsbtn.addEventListener("click", () => {
  ...
  for (let i = localStorage.length; i >= 1; i--) {
    let kL = localStorage.length - i;
    let nameOfkey = "key" + JSON.stringify(kL + 1);
    let dT = JSON.parse(localStorage.getItem(nameOfkey));
    obj_data.push({
      EmpCode: dT.EmpCode,
      EmpName: dT.EmpName,
      Att_Date: dT.Att_Date[0],
    });
  }
  const ws = XLSX.utils.json_to_sheet(obj_data);
  const wb = XLSX.utils.book_new();
